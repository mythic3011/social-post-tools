(() => {
  ...
  if (!Core || !Store || !Share) return;

  const $ = (id) => document.getElementById(id);
  const { DEFAULTS, ACTIONS, ANDROID_CAPTURE_BROWSERS, clone, loadSettings, saveSettings } = Store;
  const { status } = Share;
  ...
  globalThis.SPTPageSettings = { renderSettingsPage };
})();

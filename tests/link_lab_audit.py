...
checks = {
    ...
    'link-lab-global-settings-read-only': 'localStorage.getItem' in lab_js and 'localStorage.setItem' not in lab_js and 'localStorage.removeItem' not in lab_js,
    ...
    'link-lab-capability-provider-lists': "providersForCapability(platform.id, 'embed'" in lab_js and "providersForCapability(platform.id, 'reader'" in lab_js,
    'link-lab-provider-selectors': 'id="preview-provider"' in lab_html and 'id="reader-provider"' in lab_html and 'populateProviderSelect' in lab_js,
    'link-lab-provider-switch-local-only': 'comparison only' in lab_js,

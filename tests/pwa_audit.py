...
settings_store = (pwa/'js/settings-store.js').read_text()
install_manager = (pwa/'js/install-manager.js').read_text()
share_actions = (pwa/'js/share-actions.js').read_text()
page_settings = (pwa/'js/page-settings.js').read_text()
page_share_target = (pwa/'js/page-share-target.js').read_text()
page_capture = (pwa/'js/page-capture-handoff.js').read_text()
# Combined app JS for checks that span modules.
app_all = app + settings_store + install_manager + share_actions + page_settings + page_share_target + page_capture
...
checks = {
  ...
  'rich-capture-browser-bridge': 'openCaptureBridge' in app_all and 'capture-handoff.html' in app_all and 'Open for AI capture' in share,
  ...
  'ux-install-button-always-actionable': 'id="install-app"' in index and 'showInstallHelp()' in app_all and "manual-fallback" in app_all,
  ...
  'ux-install-share-target-diagnostic': 'id="diag-share-target"' in index and 'Share Target' in app_all,
  'ux-install-chrome-supported-path': 'Chrome is the supported Share Target registration path' in index and 'Supported path' in app_all,
  ...
  'ux-install-fallback-help': 'id="install-help"' in index and 'manual installation path' in index and 'userChoice' in app_all,
